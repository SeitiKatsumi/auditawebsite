...
export function SellerAnalysisPage() {
  ...
  return <main>
    ...
    <header className="header"><div className="container nav">
      ...
      <a className="btn btn-small desktop-cta" href="#solicitar" onClick={() => track("click_header_cta", { position: "header" })}>Analisar vendedor</a>
    ...
    <section className="hero"><Image src="/images/hero.webp" fill priority sizes="100vw" alt="Profissional usando tablet em ambiente de análise digital" className="hero-image" />
      <div className="hero-overlay"/><div className="container hero-content">
        ...
        <div className="hero-actions"><a className="btn" href="#solicitar" onClick={() => track("click_hero_cta", { position: "hero" })}>Analisar o vendedor agora <b>→</b></a><a className="btn-link" href="#como-funciona" onClick={() => track("click_how_it_works")}>Entender como funciona ↓</a></div>
    ...
    <section className="section tension"><div className="container"><div className="section-heading narrow"><p className="kicker">O risco que costuma ficar fora do radar</p><h2>O imóvel pode parecer seguro.<br/><span>A negociação pode não ser.</span></h2><p>Uma compra imobiliária envolve muito mais do que localização, preço e estado de conservação. Situações relacionadas ao vendedor precisam ser identificadas antes da decisão.</p></div>
      <div className="risk-grid">{risks.map(([n,t,d]) => <article className="risk-card" key={n}><span className="card-number">{n}</span><div className="line-icon" aria-hidden="true">⌁</div><h3>{t}</h3><p>{d}</p></article>)}</div>
    ...
    <section className="section workflow" id="como-funciona"><div className="container"><div className="section-heading"><p className="kicker">Da consulta à decisão</p><h2>Uma jornada simples para uma decisão mais informada.</h2></div><div className="steps">{steps.map((s,i)=><div className="step" key={s}><span>{String(i+1).padStart(2,"0")}</span><h3>{s}</h3><p>{["Dados básicos do vendedor iniciam a diligência.","Acessamos certidões e fontes disponíveis.","PDFs oficiais ficam centralizados.","Ocorrências são estruturadas e contextualizadas.","Um resultado objetivo apoia seus próximos passos."][i]}</p></div>)}</div><a className="btn centered" href="#solicitar">Iniciar minha análise →</a></div></section>
